...
run () {
	...
	if [ -f ./gitlab-runner-helper-images.deb ] ; then
		rm ./gitlab-runner-helper-images.deb
	fi

	if [ -f ./gitlab-runner_riscv64.deb ] ; then
		rm ./gitlab-runner_riscv64.deb
	fi
	...
	if [ -f ./gitlab-runner_${deb_arch}.deb ] ; then
		if [ -f ./gitlab-runner-helper-images_all.deb ] ; then
			reprepro -b ${repo} -C main includedeb ${suite} ./gitlab-runner_${deb_arch}.deb
			reprepro -b ${repo} -C main includedeb ${suite} ./gitlab-runner-helper-images_all.deb
		fi
	fi
}
